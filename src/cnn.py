    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(data_set):
            ...
            for i in range(len(models)):
                target = (original_target >= label_values[i]).type(
                ...
                if hyper["dice"]:
                    ...
                    loss = criterion(
                        data.type(torch.cuda.FloatTensor), target.type(torch.int64)
                    )

            dice = Dice(data, target)
            if batch_idx % 50 == 0:
                print(
                    "Evaluating Epoch: {}\tBatch: {}\tTimestamp: {}\tLoss: {}\tDice:{}".format(
                        epoch, batch_idx, time.time() - start_time, loss, dice
                    )
                )
            # if batch_idx > 2:
            #   break
...
def train_epoch(models, data_set, criterion, optimizers, epoch):
    ...
    for batch_idx, (data, original_target) in enumerate(data_set):
        ...
        for i in range(len(models)):
            ...
            if batch_idx % 20 == 0:
                print(
                    "Epoch: {}\tBatch: {}\tTimestamp: {}\tLoss: {}".format(
                        epoch, batch_idx, time.time() - start_time, loss
                    )
                )
            del loss
            del data
...
def main():
    ...
    optimizer_w = optim.Adam(
        wnet.parameters(), lr=hyper["lr"], weight_decay=hyper["weightDecay"]
    )
    optimizer_t = optim.Adam(
        tnet.parameters(), lr=hyper["lr"], weight_decay=hyper["weightDecay"]
    )
    optimizer_e = optim.Adam(
        enet.parameters(), lr=hyper["lr"], weight_decay=hyper["weightDecay"]
    )
